...
def get_prob_diff(tokenizer: PreTrainedTokenizer, mean=True):
    ...
    def prob_diff(logits, years):
        ...
        diffs = -torch.stack(diffs).to(device)
...
def generate_real_sentence(noun: str, year: int, eos: bool = False) -> str:
    century = year // 100
    sentence = f"The price of that {noun} ranges from $ {year} to $ {century}"
    if eos:
...
def real_sentence_prompt(eos: bool = False) -> List[str]:
    sentence = f"The price of that NOUN ranges from $ XX1 YY to $ XX2".split()
    if eos:
        sentence = ["<|endoftext|>"] + sentence
    return sentence
...
def generate_bad_sentence(noun: str, year: int, eos: bool = False) -> str:
    century = year // 100
    sentence = f"The price of that {noun} ranges from $ {century}01 to $ {century}"
    if eos:
        sentence = "<|endoftext|> " + sentence
    return sentence
...
def bad_sentence_prompt(eos: bool = False) -> List[str]:
    sentence = f"The price of that NOUN ranges from $ XX1 01 to $ XX2".split()
    if eos:
        sentence = ["<|endoftext|>"] + sentence
    return sentence
    ...
    def __init__(
        ...
        if isinstance(nouns, str):
            ...
            with open(nouns, "r") as f:
                ...
                noun_list = [
                    noun for noun in noun_list if len(tokenizer(noun).input_ids) == 1
                ]
        else:
            raise ValueError(
        ...
        self.good_toks, good_attn = (
            good_tokenized["input_ids"],
            good_tokenized["attention_mask"],
        )
        assert torch.all(good_attn == 1)
        bad_tokenized = tokenizer(self.bad_sentences, return_tensors="pt")
        self.bad_toks, bad_attn = (
            bad_tokenized["input_ids"],
            bad_tokenized["attention_mask"],
        )
